...
import { Scale, Loader2, CheckCircle2, SkipForward } from "lucide-react";
import { Button } from "@/components/common/Button";
import { Card } from "@/components/common/Card";
import { Select } from "@/components/common/Select";
import { Label } from "@/components/common/Input";
import BaseModal from "@/components/common/BaseModal";
import { getFontsNeedingManualLicense, assignManualFontLicense } from "@/lib/actions/font";
import { FONT_LICENSE_TYPES } from "@/lib/constants/fontLicenseTypes";

const LICENSE_OPTIONS = FONT_LICENSE_TYPES.map((v) => ({ label: v, value: v }));
...
export default function FillMissingLicensesCard({ count }: { count: number }) {
  ...
  const openModal = async () => {
    ...
    setError(null);
    try {
      const fonts = await getFontsNeedingManualLicense();
      setQueue(fonts);
    } catch (err: any) {
  ...
  const handleSave = async () => {
    if (!current || !licenseType || isSaving) return;
    setIsSaving(true);
    setError(null);
    try {
      await assignManualFontLicense(current.id, licenseType);
      setSavedCount((c) => c + 1);
      setIndex((i) => i + 1);
    } catch (err: any) {
        ...
        <BaseModal isOpen={isOpen} onClose={() => (!isSaving ? close() : undefined)} size="md">
          <BaseModal.Header onClose={() => (!isSaving ? close() : undefined)}>
            <div className="flex items-center gap-3">
              ...
              <div>
                <h3 className="text-2xl font-rezland text-black dark:text-white leading-tight">Fill Missing Licenses</h3>
          ...
          <BaseModal.Body>
            {isLoading ? (
              ...
              <div className="flex flex-col items-center justify-center py-12 gap-3 text-center">
                ...
                <p className="text-xs text-ocragray-800 dark:text-zinc-200">{savedCount} license(s) filled in.</p>
              </div>
            ) : current ? (
              <div className="space-y-4">
                ...
                <div>
                  ...
                  <Select options={LICENSE_OPTIONS} value={licenseType} onChange={setLicenseType} placeholder="Select a license" />
                </div>
