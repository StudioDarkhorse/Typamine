...
import { getFontsNeedingDafontScrape, scrapeFontFromDafont } from "@/lib/actions/font";

// Stessa regola di lib/services/dafontScraper.ts (slugifyForDafont) — non
// importata direttamente qui perché quel modulo legge anche process.env
// lato server (SCRAPER_BASE_URL), che non deve finire nel bundle client.
function dafontUrlPreview(fontName: string): string {
  const slug = fontName.trim().toLowerCase().replace(/[\s_]+/g, "-").replace(/[^a-z0-9-]/g, "").replace(/-+/g, "-");
  return `https://www.dafont.com/${slug}.font`;
}
...
// Cortesia verso lo scraper interno (nessun rate limit noto come Gemini free
// tier, ma evitiamo comunque una raffica di richieste concorrenti).
...
// "Force Dafont Scraping" in dashboard — stesso universo di candidati di
// "Fill Missing Authors"/"Fill Missing Licenses" (vedi getDafontScrapeCandidateWhere
// in lib/actions/font.ts) ma risolto provando a leggere la pagina dafont.com
// del font invece che chiedere all'admin o all'AI. Stesso schema
// modale/terminal-log di AIFontIdentityButton.
export default function ForceDafontScrapingCard({ count }: { count: number }) {
  ...
  const [results, setResults] = useState<{
    scraped: Array<{ family: string; author: string | null; licenseType: string | null }>;
    notFound: Array<{ family: string }>;
  ...
  const openModal = async () => {
    ...
    try {
      const fonts = await getFontsNeedingDafontScrape();
      setCandidates(fonts);
      setPhase("ready");
    } catch (err: any) {
      setLogs([`[ERROR] Failed to load candidate fonts: ${err.message}`]);
      setPhase("done");
      setResults({ scraped: [], notFound: [], failed: [] });
    }
  };
  ...
  const runScraping = async () => {
    setPhase("running");
    setProgress(0);
    setLogs([`Found ${candidates.length} font(s) missing author and/or license.`, "Starting dafont.com scraping session..."]);

    const scraped: Array<{ family: string; author: string | null; licenseType: string | null }> = [];
    const notFound: Array<{ family: string }> = [];
    ...
    for (let i = 0; i < candidates.length; i++) {
      ...
      setLogs((prev) => [...prev, `[${i + 1}/${candidates.length}] Scraping ${dafontUrlPreview(font.name)}...`].slice(-40));

      try {
        const result = await scrapeFontFromDafont(font.id);

        if (result.notFound) {
          notFound.push({ family: font.name });
          setLogs((prev) => [...prev, `⚠ ${font.name} → not found on dafont.com (404)`].slice(-40));
        } else {
          scraped.push({ family: result.family, author: result.author, licenseType: result.licenseType });

          // Applicato (scritto su DB) vs trovato sulla pagina ma tenuto
          // com'era (il font aveva già un valore reale) — due casi distinti,
          // mai da confondere con "niente trovato sulla pagina".
    ...
    setLogs((prev) => [
      ...prev,
      "-------------------------",
      `Done. Scraped ${scraped.length}, not found ${notFound.length}, failed ${failed.length}.`,
    ]);
    setResults({ scraped, notFound, failed });
    setPhase("done");

    if (typeof Notification !== "undefined" && Notification.permission === "granted") {
      new Notification("Dafont scraping complete", {
        body: `Updated ${scraped.length} font(s)${notFound.length ? `, ${notFound.length} not found` : ""}${failed.length ? `, ${failed.length} failed` : ""}.`,
      });
    }
  };
      ...
      <Card roundness="lg" visualHover className="cursor-pointer" onClick={openModal}>
        <div className="p-2 h-full flex flex-col justify-center items-center gap-2">
          <p className="text-xl font-bold text-black dark:text-white truncate text-center">
            {count} font{count !== 1 ? "s" : ""} still missing Infos
          </p>
      ...
      {isOpen && (
        <BaseModal isOpen={isOpen} onClose={() => (phase !== "running" ? close() : undefined)} size="lg">
            ...
            {phase === "ready" && (
              <div className="space-y-4">
                <p className="text-xs text-zinc-600 dark:text-zinc-400 font-semibold">
                  ...
                  missing a real author, a license type, or both. Each will be looked up on its dafont.com page
                  (name → slug, e.g. "Matcha Mint" → matcha-mint.font) — whatever is found is saved directly on the font.
                </p>
                {candidates.length > 0 && (
              ...
              <div className="space-y-6">
                ...
                <div className="rounded-xl bg-zinc-950 border border-zinc-800 overflow-hidden">
                  <div className="flex items-center gap-2 px-4 py-2.5 border-b border-zinc-800 bg-zinc-900/50">
                    ...
                    <span className="text-[10px] font-bold text-zinc-500 ml-2 uppercase tracking-widest">Dafont Scrape Log</span>
                  </div>
                  <div
                    ref={terminalRef}
                  ...
                  <div className="space-y-3">
                    <div className="grid grid-cols-3 gap-3">
                      <div className="rounded-xl bg-emerald-50 dark:bg-emerald-950/30 border border-emerald-200 dark:border-emerald-900/50 p-4 text-center">
                        <span className="text-[10px] font-bold uppercase tracking-widest text-emerald-700 dark:text-emerald-400 block mb-1">Scraped</span>
                        <span className="text-3xl font-black text-emerald-600 dark:text-emerald-400 leading-none">{results.scraped.length}</span>
                      </div>
                      <div className="rounded-xl bg-amber-50 dark:bg-amber-950/30 border border-amber-200 dark:border-amber-900/50 p-4 text-center">
                        <span className="text-[10px] font-bold uppercase tracking-widest text-amber-700 dark:text-amber-400 block mb-1">Not Found</span>
                        <span className="text-3xl font-black text-amber-600 dark:text-amber-400 leading-none">{results.notFound.length}</span>
                      </div>
          ...
          <BaseModal.Footer>
            {phase === "ready" && (
              <div className="flex gap-2 w-full">
                ...
                >
                  ...
                  {candidates.length === 0 ? "Nothing to scrape" : `Scrape ${candidates.length} font(s)`}
                </Button>
              </div>
            )}
            {phase === "running" && (
              <div className="flex gap-2 w-full">
                <Button variant="secondary" size="md" roundness="md" disabled={isCancelling} onClick={handleCancel} className="font-bold">
                  {isCancelling ? "Cancelling..." : "Cancel"}
                </Button>
                <Button variant="secondary" size="md" roundness="md" disabled fullWidth className="flex items-center justify-center gap-2 font-bold">
            ...
            {phase === "done" && (
              <Button variant="primary" size="md" roundness="md" onClick={close} fullWidth className="flex items-center justify-center gap-2 font-bold">
